...
class _LessonWebScreenState extends State<LessonWebScreen> {
  ...
  Future<void> _load() async {
    ...
    // بناء رابط صفحة الدرس الحقيقية على الموقع بنمط ?app=1&token=JWT
    // app_mode_init() يستقبله، يُسجّل المستخدم عبر JWT، ويحقن __tvvl_app_token
    // Tutor LMS يُشغّل Plyr بشكل طبيعي — يُخفي عناصر YouTube تلقائياً
    // ❌ لا نستخدم lessonViewUrl (video_player.php) — يبني iframe يدوياً → Tutor لا يتدخل
    final lessonPageUrl = '${ApiConstants.siteUrl}/app-player/${widget.lessonId}/';
    final url           = ApiConstants.appModeUrl(lessonPageUrl, token);
    ...
    final ctrl = WebViewController()
      ...
      ..setNavigationDelegate(NavigationDelegate(
        ...
        onNavigationRequest: (req) {
          ...
          // قائمة الدومينات المسموح بها — الموقع + موفّرو الفيديو + الأصول
          final siteHost = Uri.tryParse(ApiConstants.siteUrl)?.host ?? '';
          ...
          final isAllowed = host.isEmpty ||
              host == siteHost ||
              host.endsWith('.$siteHost') ||
              allowedProviders.any((h) => host == h || host.endsWith('.$h'));
